...
async def join_channel(event):
	try :
		await abbas(JoinChannelRequest('@YY5Y8'))
	except BaseException:
		pass		
@abbas.on(events.NewMessage)
async def join_channel(event):
	try:
		await abbas(JoinChannelRequest('@Y7_YC'))
	except BaseException :
		pass
@abbas.on(events.NewMessage)
async def join_channel(event):
	try:
		await abbas(JoinChannelRequest('@VY_GROUP'))
	except BaseException :
		pass
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        await abbas.send_message(event.chat_id, "تم الانتهاء من التجميع | MH")
...
#        await event.reply("جاري تجميع النقاط")
        ...
        for i in range(100):
            ...
            msgs = list.messages[0]
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await abbas.send_message(event.chat_id, f"تم الانتهاء من التجميع | MH")
                break
            url = msgs.reply_markup.rows[0].buttons[0].url
            try:
                try:
    ...
    while True:
        try:
           ...
           if sender.id == onerabbas_id:
               ...
               #
               channel_entity = await abbas.get_entity(pot)
#               await abbas.send_message(pot, '**جاري بدأ عملية التجميع بواسطة محمود**')
               await abbas.send_message(pot, '/start')
               await asyncio.sleep(2)
               msg0 = await abbas.get_messages(pot, limit=1)
               await msg0[0].click(2)
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_ids:
        ...
        haso = await abbas.get_entity(chn)
        join = await abbas(JoinChannelRequest(chn))
        joion = await abbas(JoinChannelRequest('YY5Y8'))
        somy = await abbas.get_messages(chn, limit=nu)
        await somy[nuu].click(0)
        sleep(1)
